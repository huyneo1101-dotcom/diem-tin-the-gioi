...
⛔ **CHỈ CÒN 01 PHIÊN/NGÀY: SÁNG SỚM.** Phiên TỐI bỏ hẳn 18/09/2026, chỉ thị Huy: *"chỉ cần gửi tin 4h sáng thôi, không phải quét và gửi buổi tối nữa đâu"*. Mày là phiên SÁNG SỚM (fire 03:47/04:47 giờ VN, ô khoá `sang`) — đêm VN là ngày làm việc Mỹ nên nhiều tin mới; nhãn `state.py` có thể in "CHAY BU (sang som)", kệ nhãn cũ, đây là phiên chủ động hợp lệ. Bảng lịch thật: `docs/LICH.md`. Ghi log dùng chữ "phien sang som". Commit mẫu `Cap nhat ban tin ...` → email tự gửi. **Phiên SÁNG SỚM sau khi xong bản tin còn làm TIẾP pipeline `event-scan` (sự kiện/tập trận/think-tank) trong CÙNG phiên này — xem BƯỚC 6 cuối file.**

⚠️ Vẫn gọi `TZ='Asia/Ho_Chi_Minh' date +%H:%M` ở Bước 1 rồi để `state.py claim` tự chọn ô. Nếu giờ VN ≥ 14:00 thì `claim` chọn ô `toi` và trả **exit 12 (SAI GIỜ)** vì khung ca tối không còn mốc nào — ghi một dòng SKIP vào log rồi KẾT THÚC ÊM, ⛔ TUYỆT ĐỐI KHÔNG thêm `--bo-cong-gio` để lách.
...
1. `git pull --rebase origin main` rồi `python3 scripts/state.py claim web-scan`.
   ...
   - ⛔ **NGOẠI LỆ DUY NHẤT của exit 10 — CỜ ĐÃ XONG NHƯNG SỔ ĐÃ GỬI CHƯA CÓ DÒNG CỦA CA NÀY** (đúc 29/07/2026, sự cố thật; luật song sinh với `docs/routine-web-scan.md` mục "PHIÊN TỐI — BỐI CẢNH RIÊNG" điều 3, nay là nhật ký vấp — cơ chế vẫn áp cho ca sáng vì mốc CI 04:47 là lớp cuối và khi máy Mac ngủ thì không còn ai đứng sau nó). Gặp exit 10 thì làm ĐỦ 3 lệnh phẳng này trước khi SKIP:
     ...
     | Điều kiện | Làm gì |
     |---|---|
     | `--xem` CÓ dòng ngày hôm nay kèm `[sang]` | SKIP êm như trên. Bản tin đã tới tay |
...
3. Quét theo SKILL. Sau mỗi mốc lớn: ghi checkpoint log + `python3 scripts/beat_push.py web-scan`.
   ⛔ **DÙNG ĐÚNG `beat_push.py`, KHÔNG BAO GIỜ gọi `state.py beat` trần** (vá 20/09/2026, sự cố thật đêm 19/09 — `logs/scan-2026-09-19.log` dòng `[21:45Z]`): `state.py beat` chỉ ghi nhịp tim CỤC BỘ trong container CI, còn việc ĐẨY lên git là một lệnh `git` RIÊNG — phiên đêm đó gọi `beat` cục bộ đúng lúc (04:31, 04:41 giờ VN) nhưng KHÔNG đẩy kịp, máy Mac chỉ thấy nhịp tim cũ nhất (04:04) qua git, tính stale >30 phút rồi giành khoá quét chồng, phí ~12 triệu token. `beat_push.py` gộp cứng ghi-nhịp-tim + đẩy git thành MỘT lệnh không tách rời được — không cần tự nhớ "nhớ push sau khi beat" nữa, và nó tự lo cả việc pull/rebase an toàn khi bị phiên khác chen (không dùng `pull --rebase` trần, xem docstring script).
   ...
   Vì vậy beat ở CẢ BA chỗ này, không chỉ ở mốc lớn: **(a) ngay sau `harvest.py` + `telegram_harvest.py`** · **(b) ngay TRƯỚC khi giao lô agent** · **(c) sau khi gom xong kết quả agent**. Nguyên tắc: **hai nhịp liên tiếp không được cách quá ~15 phút**; sắp làm việc gì dự kiến lâu thì beat trước khi bắt đầu, không phải sau khi xong. **5 agent chủ đề giao SONG SONG trong MỘT lô là đúng khe hở đêm 19/09** (lô agent chạy ~40 phút không có cách nào chen một beat vào GIỮA lúc đang chờ 5 agent trả lời) — beat (b) + (c) ở ĐÚNG hai đầu của lô là đủ MIỄN LÀ đã dùng `beat_push.py` (đẩy thật), không phải beat trần.
3b. **Dùng file Jay Lâm làm BỘ LỌC, sau khi nạp tin quét, TRƯỚC khi commit** (đảo nguyên tắc 01/08/2026, nguyên văn Huy: *"file của Jay Lâm gửi chỉ là để so sánh xem có tin nào mày quét được mà bị trùng với tin trong file đó không thôi"* · *"nếu có tin bị trùng thì tự xoá khỏi tổng hợp tin đã quét đi"*).
...
2. Quét sự kiện + tập trận bằng agent (tool Agent, model "sonnet"): nhúng nguyên output
   ...
   `python3 scripts/add_news.py /tmp/new_items_event.json`. Beat: `python3 scripts/beat_push.py event-scan`
   NGAY TRƯỚC khi giao agent, hai nhịp không cách quá ~15 phút. ⛔ Dùng đúng `beat_push.py`,
   KHÔNG `state.py beat` trần — lý do xem BƯỚC 3 của web-scan phía trên (vá 20/09/2026).
