...
3b. BÀI THINK-TANK (mỗi phiên sáng, KHÔNG chỉ Chủ nhật) — mục 🧠 Phân tích → 🏛️ Think-tank trên web.
   - `python3 scripts/add_analyses.py --candidates` → danh sách ứng viên từ RSS 13 viện đã verify (đã tự bỏ bài đã có trong DATA + đường dẫn rác `/in-the-news/`).
   - Giao agent Sonnet CHỌN 4–6 bài đúng gu: ưu tiên Úc/AUKUS · Biển Đông · răn đe hạt nhân/CNQS · Mỹ–Trung–Đài Loan · Mali/Sahel; LOẠI bài chính trị xã hội nội bộ Mỹ (phá thai, hôn nhân, giáo dục, toà án thuần), bài quảng bá viện, điểm sách. Agent phải MỞ ĐỌC từng bài bằng WebFetch rồi viết tiếng Việt: `title` (dịch thoát) · `summary` 2–3 câu kèm số liệu thật · `takeaway` 1–2 câu điều rút ra · `topic` · `region` · `author` · `outlet` · `date` (ngày đăng THẬT trên bài, RSS đôi khi lệch 1 ngày). Số liệu nào mập mờ/lỗi ký tự thì BỎ, KHÔNG đoán.
