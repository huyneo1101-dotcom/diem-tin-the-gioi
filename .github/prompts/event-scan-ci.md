...
3. BỐI CẢNH + KHÁI NIỆM tập trận: với mỗi cuộc tập trận MỚI vừa tạo VÀ mỗi cuộc đang diễn ra CHƯA có `background` — giao agent Sonnet viết `background` (2–4 câu bối cảnh chiến lược, nhiều đoạn ngăn `\n`) + `concepts` (3–6 thuật ngữ, [{term,def}] def 1 câu). Ghi `/tmp/briefing.json` = [{"name":"<khớp đúng name>","background":"...","concepts":[...]}] rồi `python3 scripts/set_exercise_briefing.py /tmp/briefing.json`. Không viết lại cho cuộc đã có background.
   ⤷ Web hiện Bối cảnh/Khái niệm bằng 2 NÚT → TRANG RIÊNG đọc toàn văn (không còn ô tóm tắt inline). `background` (đoạn ngắn agent sinh) chỉ là FALLBACK; nội dung ĐẦY ĐỦ lấy từ **file Word "thông tin nền"** của quy trình SA BÀN — tên `<Tên> Background vN.docx`, nằm CÙNG folder bài phân tích ở `~/Desktop/QD/<Tập trận>/` (KHÁC bài phân tích INTREP — file nền có 2 phần: PHẦN I bối cảnh các bên → `backgroundDoc`; PHẦN II khái niệm → `concepts`). ⚠️ ĐỪNG lấy file bài phân tích (`*Claude vN*`, `*FINAL*`, `format-mau`) — đó là bài phân tích có mục đích/đánh giá/khuyến nghị, KHÔNG phải thông tin nền. Nạp bằng: `python3 scripts/import_background_docx.py "<file Background.docx>" "<tên exercise khớp DATA>"` → sinh `/tmp/briefing_bg.json` → `python3 scripts/set_exercise_briefing.py /tmp/briefing_bg.json`. **Bước này CHẠY TAY ở máy local** (file nền nằm trên Desktop, CI GitHub không thấy).
