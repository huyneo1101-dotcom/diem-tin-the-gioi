...
## Hai pipeline độc lập — cờ riêng, KHÔNG dùng chung `generatedAt` (sửa 21/07/2026)
Có 2 đường nạp tin, chạy song song và BỔ SUNG cho nhau:

| Pipeline | Ai chạy | Giờ VN | Nạp gì |
|---|---|---|---|
| `drive-import` | **GitHub Action** `import-news-from-drive.yml` | 08:00 & 20:00 | File `ban-tin-chien-luoc-*.json` upload lên Drive |
| `web-scan` | **Routine Claude** (6 agent Sonnet) | 19:00 → 20:00 → 21:00 | Quét web trực tiếp: worldNews/usNews/xNews/tập trận/sự kiện ngoại giao |
...
```
python3 scripts/state.py check web-scan     # RUN (exit 0) hoặc SKIP (exit 10)
...
python3 scripts/state.py show                              # xem cả 2 pipeline
```
Chỉ `done` mới đẩy `lastSuccessDate` → chỉ khi thật sự nạp được tin mới chặn lần fire kế tiếp; `fail`/`skip` để lần sau quét lại.
...
## Đồng bộ "Bài đã lưu" Báo Mới (độc lập với quy trình quét tin)
...
`baomoi-saved.json`. Web (`loadBaomoi` trong `index.html`) fetch file này lúc tải trang
và **trộn thẳng vào `DATA.worldNews`** (hiện ở tab Bài mới/Thế giới, gắn nhãn 📌 Đã lưu) —
KHÔNG có tab riêng, KHÔNG phân tích sở thích. Cần secret `BAOMOI_COOKIE` (cookie đăng nhập).
