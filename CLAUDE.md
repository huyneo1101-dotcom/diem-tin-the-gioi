...
## Chỉ tiêu số lượng mỗi lần quét (BẮT BUỘC — điều chỉnh 13/07/2026 cho khung 2 NGÀY)
Vì **chỉ quét 2 ngày gần nhất** (hôm nay + hôm qua), sản lượng thực tế THẤP — đây là chỉ tiêu **thực tế, best-effort**: đủ thì lấy, thiếu thì thôi (KHÔNG nới ngày/bộ lọc để nhồi). Vẫn **dồn ưu tiên cho chủ đề THÍCH**.
| Phần (mỗi mục world + us) | Chỉ tiêu thực tế |
|---|---|
...
| **08:05** / 20:05 | Action `sync-baomoi.yml` | Cập nhật `baomoi-saved.json` (bài đã lưu Báo Mới) |
| **08:15** / 20:15 | Scheduled task local `web-scan` (6+1 agent Sonnet) | Quét web + nạp Báo Mới vào `DATA` → publish |
| 09:15 / 21:15 | Scheduled task local `web-scan` (dự phòng) | Chỉ chạy nếu mốc chính chết; cờ `state.json` làm nó tự no-op khi đã xong |
...
- **Retry cho tới khi xong**: Routine fire lúc 08:15 & 20:15 VN (mốc chính) + 09:15 & 21:15 (dự phòng) — cron `15 1,2,13,14 * * *` UTC. Nhờ bước idempotent, khi mốc chính đã DONE thì lần dự phòng tự no-op; chỉ khi phiên chính chết (session limit, mạng...) thì lần sau mới quét lại. Vẫn đúng **2 bản tin/ngày**, lần dự phòng chỉ là lưới an toàn. (Scheduler tối thiểu 1 giờ/lần — không đặt 15 phút được.)
...
## "Bài đã lưu" Báo Mới — nạp THẲNG vào `DATA` khi quét (đổi 21/07/2026)
GitHub Action `sync-baomoi.yml` kéo danh sách bookmark trong tài khoản Báo Mới, **lọc
chỉ giữ bài đúng 4 chủ đề của web** (Kinh tế/Chính trị/Công nghệ quân sự/Ngoại giao) →
`baomoi-saved.json`. File này chỉ có `date/category/title/sourceName/sourceUrl/region`,
**thiếu `summary` + `significance`** (2 field guardrail bắt buộc).

**Phiên quét (Agent 7) gộp chúng vào `DATA`:**
```
python3 scripts/add_news.py --baomoi-pending    # bài đã lưu CHƯA có trong DATA
```
→ giao 1 agent Sonnet viết `summary` + `significance` (sửa `category`/`region` nếu phân loại
tự động sai) → trả về section **`baomoiNews`** trong `/tmp/new_items.json` → script chèn vào
`DATA.worldNews` kèm cờ `_baomoi` (web gắn nhãn 📌 Đã lưu).
**`baomoiNews` KHÔNG bị kiểm tra khung 2 ngày** — người dùng tự tay bookmark, bài có thể đăng
từ hàng tháng trước; chặn theo ngày thì loại sạch. Các kiểm tra khác vẫn giữ nguyên.
...
tự bỏ qua (nó dedupe theo tiêu đề). Không có tab riêng, KHÔNG phân tích sở thích.
Cần secret `BAOMOI_COOKIE` (cookie đăng nhập).
...
## Ghi chú vận hành
- **Routine `web-scan` nằm ở ĐÂU** (đổi 22/07/2026): là **scheduled task LOCAL trên máy Mac của Huy**, không phải routine trên claude.ai. `taskId: web-scan-diem-tin`, file `~/.claude/scheduled-tasks/web-scan-diem-tin/SKILL.md`, cron `15 8,9,20,21 * * *` — **giờ LOCAL (Asia/Ho_Chi_Minh), không phải UTC**. Quản lý ở mục "Scheduled" trên sidebar, hoặc bằng tool `mcp__scheduled-tasks__*`. Mỗi lần fire tạo session mới, tự đọc file này để lấy quy tắc, có log + idempotent (`state.py check web-scan`) + fire dự phòng.
- **HẠN CHẾ phải biết**: scheduled task local **chỉ chạy khi app Claude đang mở**; app đóng lúc tới giờ thì nó chạy bù ở lần mở kế tiếp. Không còn routine server-side nào dự phòng (routine cũ trên claude.ai đã mất khi đổi máy). Máy ngủ lúc 08:15 → bản tin sáng trễ tới lúc mở máy, KHÔNG phải lỗi code — kiểm tra `state.py show` trước khi đi truy bug.
