...
GitHub Action `sync-baomoi.yml` kéo danh sách bookmark trong tài khoản Báo Mới về
`baomoi-saved.json` + phân tích sở thích (`baomoi-analysis.json`, `baomoi-preferences.md`),
hiển thị ở tab **📌 Báo Mới** trên web. Cần secret `BAOMOI_COOKIE` (cookie đăng nhập).
Khi Action fail vì cookie hết hạn (err -801) → làm mới cookie theo `docs/baomoi-sync.md`.
Endpoint chỉ cần cookie, KHÔNG kiểm tra `sig` (đã kiểm chứng 18/07/2026).
