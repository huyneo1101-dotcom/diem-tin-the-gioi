...
## Chỉ tiêu số lượng mỗi lần quét (BẮT BUỘC — SÀN CỨNG, chỉ thị người dùng 23/07/2026)
**SÀN CỨNG mỗi PHIÊN quét: `worldNews` (Quốc tế) ≥ 10 tin · `usNews` (Mỹ) ≥ 10 tin — CHẤT LƯỢNG CAO.**
**CHƯA ĐỦ THÌ CHƯA DỪNG:** thiếu mục nào thì giao thêm agent Sonnet bổ sung riêng mục đó (chỉ rõ nguồn/góc CHƯA khai thác), chạy lại `add_news.py`, LẶP cho tới khi CẢ HAI mục đạt ≥10. Đây là sàn áp cho MỖI phiên (sáng riêng, tối riêng), không phải tổng ngày.

"Chất lượng cao" = qua guardrail + đúng nguồn 3 tầng + đúng bộ lọc sở thích + link thẳng bài gốc trong khung 2 ngày. **KHÔNG hạ chuẩn để nhồi cho đủ số**: không bịa tin/link, không lấy tin cũ hơn hôm qua, không nhét tin rác. Nội bộ Mỹ đã mở toàn bộ (xem bộ lọc) nên nguồn tin Mỹ rất dồi dào — 10+10 thừa sức đạt trong khung 2 ngày nếu chịu khó giao thêm agent.

Phân bổ GỢI Ý trong mỗi mục ≥10 (linh hoạt, miễn tổng mục đạt sàn):
| Category | Gợi ý mỗi mục (world / us) |
|---|---|
| **Công nghệ quân sự** | 3–5 tin (chủ đề thích nhất — khí tài/hệ thống cụ thể) |
| **Ngoại giao** | 3–5 tin (hiệp định/khuôn khổ an ninh–QP, thượng đỉnh có kết quả) |
| **Kinh tế** | 2–3 tin (vĩ mô/chính sách/chuỗi cung ứng chiến lược) |
| **Chính trị** | 2–3 tin (world: thể chế/chiến lược great-power · **us: MỞ TOÀN BỘ nội bộ Mỹ** — dư địa lớn nhất để bù cho đủ 10) |
| 🎯 **Trọng tâm chủ động** | **Úc · Biển Đông · Nội bộ Mỹ** — nằm rải trong 4 category trên, mỗi trọng tâm 1–2 tin/phiên nếu có |

| Phần khác | Chỉ tiêu (không tính vào sàn 10+10) |
|---|---|
...
→ Tổng **≥20 tin bản tin/phiên** (10 world + 10 us) + xNews + sự kiện. **Fallback bất khả kháng:** chỉ khi đã giao ≥3 vòng agent bổ sung mà vẫn không đủ tin SẠCH (ngày cực khan / môi trường lỗi) mới chấp nhận thiếu — ghi RÕ trong tóm tắt còn thiếu bao nhiêu và vì sao. Thiếu vì lười giao thêm agent thì KHÔNG chấp nhận. Tuyệt đối không bịa để lấp.
...
5. Chèn vào `index.html` bằng script có sẵn, KHÔNG dùng Edit/Write trực tiếp lên `index.html`:
   ...
   - **KIỂM TRA SÀN CỨNG:** script in ra tổng `worldNews` và `usNews` đã có trong DATA hôm nay. Nếu mục nào **< 10 tin của phiên này**, **giao thêm agent Sonnet bổ sung** riêng mục đó — chỉ rõ category còn thiếu + nguồn/góc CHƯA khai thác (nội bộ Mỹ mở toàn bộ là dư địa lớn nhất) — rồi chạy lại script. **LẶP cho tới khi cả world lẫn us ≥ 10.** Script cộng dồn an toàn (không tạo trùng vì mỗi lần chỉ gồm tin mới). CHỈ dừng khi đạt sàn HOẶC đã ≥3 vòng bổ sung mà thật sự cạn tin sạch (ghi rõ trong log + tóm tắt).
...
| **09:15** / 20:15 | Scheduled task local `web-scan` (7+1 agent Sonnet) | Quét web + nạp Báo Mới vào `DATA` → publish |
| 10:15 / 21:15 | Scheduled task local `web-scan` (dự phòng) | Chỉ chạy nếu mốc chính chết; cờ `state.json` làm nó tự no-op khi đã xong |
...
- **Retry cho tới khi xong**: Routine fire lúc 09:15 & 20:15 VN (mốc chính) + 10:15 & 21:15 (dự phòng) — cron `15 9,10,20,21 * * *` giờ LOCAL. Nhờ bước idempotent, khi mốc chính đã DONE thì lần dự phòng tự no-op; chỉ khi phiên chính chết (session limit, mạng...) thì lần sau mới quét lại. Vẫn đúng **2 bản tin/ngày**, lần dự phòng chỉ là lưới an toàn. (Scheduler tối thiểu 1 giờ/lần — không đặt 15 phút được.)
...
## Ghi chú vận hành
- **Routine `web-scan` nằm ở ĐÂU** (đổi 22/07/2026): là **scheduled task LOCAL trên máy Mac của Huy**, không phải routine trên claude.ai. `taskId: web-scan-diem-tin`, file `~/.claude/scheduled-tasks/web-scan-diem-tin/SKILL.md`, cron `15 9,10,20,21 * * *` — **giờ LOCAL (Asia/Ho_Chi_Minh), không phải UTC**. Quản lý ở mục "Scheduled" trên sidebar, hoặc bằng tool `mcp__scheduled-tasks__*`. Mỗi lần fire tạo session mới, tự đọc file này để lấy quy tắc, có log + idempotent (`state.py check web-scan`) + fire dự phòng.
- **HẠN CHẾ phải biết**: scheduled task local **chỉ chạy khi app Claude đang mở**; app đóng lúc tới giờ thì nó chạy bù ở lần mở kế tiếp. Không còn routine server-side nào dự phòng (routine cũ trên claude.ai đã mất khi đổi máy). Máy ngủ lúc 09:15 → bản tin sáng trễ tới lúc mở máy, KHÔNG phải lỗi code — kiểm tra `state.py show` trước khi đi truy bug.
