...
5. **Điều hướng theo sở thích người đọc.** Người đọc bấm 👍/👎 trên từng tin (tab ⭐ Đã lưu → 📊 Sở thích), đồng bộ lên Supabase. Mỗi lần quét, session **đọc tổng hợp sở thích** để ưu tiên (điểm dương) / giảm ưu tiên (điểm âm) chuyên mục · khu vực · nguồn — chi tiết cách lấy ở `preferences.md`: (A) tự động — WebFetch view công khai `vote_stats` (chỉ số đếm, không lộ danh tính, đọc bằng publishable key); (B) fallback — bảng trọng số tay trong `preferences.md` (cập nhật từ file `diemtin-sothich.json` người đọc export). Đây là **định hướng mềm**: vẫn giữ tối thiểu 2 tin/category, không bỏ hẳn mục nào, không ghi đè quy tắc nguồn 3 tầng/chất lượng. Schema: `docs/supabase-setup.sql`.
