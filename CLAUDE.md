...
## Chỉ tiêu số lượng mỗi lần quét (BẮT BUỘC — điều chỉnh theo sở thích 12/07/2026, ưu tiên NHIỀU tin đúng gu)
Mục tiêu mới: **RA NHIỀU tin/ngày đúng sở thích** (xem `preferences.md`). Tăng sản lượng cho chủ đề THÍCH, giữ tối thiểu cho chủ đề ít thích.
| Phần | Chỉ tiêu |
|---|---|
| `worldNews` — **Công nghệ quân sự** | **4–6 tin** (chủ đề thích nhất — khí tài/hệ thống cụ thể) |
| `worldNews` — **Ngoại giao** | **4–6 tin** (hiệp định/khuôn khổ an ninh–QP, thượng đỉnh có kết quả) |
| `worldNews` — **Kinh tế** | **3–4 tin** (CHỈ vĩ mô/chính sách/chuỗi cung ứng chiến lược) |
| `worldNews` — **Chính trị** | **2 tin** (CHỈ thể chế/luật/chiến lược great-power — xem bộ lọc dưới) |
| `usNews` (4 category) | Tương tự trọng số trên → tổng ~12–16 tin |
| `xNews` | 4–5 tin (ưu tiên tài khoản QP/an ninh/chính thức) |
| `exercises` (tập trận) | 1–2 tin cập nhật (sự kiện `ongoing`) |
| `dipEvents` (ngoại giao) | 1–2 tin cập nhật + tạo mới nếu có sự kiện lớn |

→ Tổng mục tiêu **~28–40 tin/ngày** (trước đây ~16–24). Ưu tiên Công nghệ quân sự + Ngoại giao.

### Bộ LỌC SỞ THÍCH (bắt buộc — nhúng vào mọi agent; nguồn: `preferences.md`)
...
**Nguyên tắc "cứu":** tin công ty/chính trị VẪN nhận nếu gắn chủ đề chiến lược (vd Boeing↔máy bay quân sự, Samsung↔chip AI). Khu vực KHÔNG phải trục ưu tiên — chọn theo chủ đề/kiểu tin.
