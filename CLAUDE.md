...
| **08:15** / 20:15 | Routine Claude `web-scan` (6+1 agent Sonnet) | Quét web + nạp Báo Mới vào `DATA` → publish |
| 09:15 / 21:15 | Routine Claude `web-scan` (dự phòng) | Chỉ chạy nếu mốc chính chết; cờ `state.json` làm nó tự no-op khi đã xong |
...
## Ghi chú vận hành
- Routine `web-scan` tự động chạy lúc 19:00 → 20:00 → 21:00 giờ VN (`0 12-14 * * *` UTC), tạo session mới mỗi lần, tự đọc file này để lấy quy tắc, có log + idempotent (`state.py check web-scan`) + retry (xem mục trên).
