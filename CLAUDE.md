...
5. **Điều hướng theo sở thích người đọc (`preferences.md`).** Người đọc bấm 👍/👎 trên từng tin (tab ⭐ Đã lưu → 📊 Sở thích), export ra `diemtin-sothich.json`; agent cập nhật trọng số chuyên mục/khu vực/nguồn vào `preferences.md`. Mỗi lần quét ĐỌC file này: điểm dương → ưu tiên hơn, điểm âm → giảm ưu tiên. Đây là **định hướng mềm**: vẫn giữ tối thiểu 2 tin/category, không bỏ hẳn mục nào, không ghi đè quy tắc nguồn 3 tầng/chất lượng. (Votes nằm ở localStorage trình duyệt người đọc — session quét không đọc trực tiếp được; cầu nối là export JSON → `preferences.md`, hoặc sau này Supabase.)
