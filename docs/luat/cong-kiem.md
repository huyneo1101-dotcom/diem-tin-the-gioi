...
| `tests/test-tach-analyses.py` | Việc tách kho think-tank ra `data/analyses.json` (30/07/2026) | 9 ca — mọi mắt xích đều hỏng-thì-im-lặng: mục Think-tank trống · 442 nhãn MỚI · guardrail trùng-url tê liệt · offline mất kho. `--tu-kiem` dựng **6 bản hỏng**. ⚠️ Ca *lời gọi ở luồng boot* nay nhận CẢ hình dạng gọi gián tiếp: từ commit 704c880 (21/08/2026) boot gọi `napKhiRanh()` rồi hàm đó mới gọi `loadAnalyses()`, nên bản kiểm cũ neo vào `^loadAnalyses();` báo đỏ oan liên tục 21→25/08 — mà ca đỏ trên bản thật làm `--tu-kiem` thoát sớm, tức CẢ 09 ca ngừng phát hiện trong im lặng. Hai bản hỏng canh hai mắt của đường đi: gỡ `loadAnalyses()` khỏi hàm bọc, và gỡ lời gọi hàm bọc ở boot |
...
| `tests/test-bang-nguon-claude-md.py` | Đường ĐỌC BẢNG NGUỒN từ CLAUDE.md + phép lấy TIÊU ĐỀ của lớp `[HTML]` | 15 ca — 7 PHẢI CHẶN (nhắc tên bảng trong văn xuôi ×1/×3 · bảng HTML lọt vào lớp RSS · feed giao với trang HTML · thẻ `<a>` gộp tóm tắt vẫn phải ra tiêu đề qua `aria-label` · và qua `<h4 class=title>` kèm tiêu đề phải sạch), 7 đối chứng (cột `CI` bị bỏ ở local · đủ 06 trang quân chủng · Navy+Marines có ở local · tên không mang dấu `**` · không có bảng thì trả rỗng êm · tiêu đề đổi chữ vẫn đọc được · **chống nới tay**: không có nguồn tiêu đề sạch thì BỎ chứ không nạp tiêu đề rác), và 2 ca cho trường hợp **mục nguồn đứng CUỐI file** — cả lớp RSS lẫn bảng trang HTML phải còn nguyên, vì trước 25/08/2026 ba script bóc bảng trả RỖNG khi không còn tiêu đề `##` nào phía sau. `--tu-kiem` bắt 6/6 bản hỏng |
...
| `tests/test-nap-file-coquan.py` | Nạp file `.docx` cơ quan gửi làm bộ lọc (`tin_jaylam.py --nap-file` · `boc_docx` · `chuan_url` · `nap_file`) | **35 ca (13 PHẢI CHẶN) · `--tu-kiem` bắt 11/11 bản hỏng** — bóc link hai đường (hyperlink + dán trần) · **không cắt `)` khi ngoặc trong đường dẫn cân bằng** (lỗi thật bắt được lúc dựng: link Ballotpedia thành link ma) · đoạn không chữ vẫn giữ tin · PHẢI CHẶN: không phải zip · thiếu `word/document.xml` · **bóc ra 0 link** · nhãn ngoài 3-60 · `id_jay` là nhãn BỊA. **`chuan_url` giữ chuỗi truy vấn mang nghĩa** (cắt sạch là gộp nhầm hai bài, XOÁ OAN một tin) · sổ loại giữ url NGUYÊN VĂN · `--thu` không chạm sổ |
